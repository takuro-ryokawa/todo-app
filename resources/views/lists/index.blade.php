...
<body class="text-gray-700 p-10">
    <h2 class="text-left p-3">右下の＋ボタンからTodoを追加してください</h2>
    <div class="grid grid-cols-2 gap-10 m-10 md:grid-cols-3 lg:grid-cols-4">
    @foreach($lists as $list)
        <a href="{{ route('todos.index', ['list' => $list->id]) }}"
            class="flex flex-col aspect-square border-slate-700 p-6 shadow-lg hover:bg-gray-50">
            <div class="flex-1">
            ...
            <ul>
                ...
                <!-- <li>{{ $todo->body }}</li> -->
                 <li>
                    ...
                    <span class="{{ $todo->flag ? 'line-through text-gray-400' : ''}}">{{ $todo->body }}</span>
                 </li>
                @endforeach
            </ul>
            </div>
                <div class="flex justify-between">
                <p class="text-right">{{ $list->updated_at->format('Y-m-d H:i') }}</p>
                <form action="{{ route('lists.destroy', ['list' => $list->id]) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        class="items-center text-sm font-medium text-center hover:text-red-700">削除</button>
                </form>
                </div>
        </a>
    @endforeach
